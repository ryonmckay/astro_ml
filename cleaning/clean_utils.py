...
def clean_df(df):
    ...
    # Loop over all columns that end with '_retrograde' only
    # - these had to be forced int separately because they had mixed values: e.g. float and object empty cells
    #   and weren't reacting to the classic method
    for col in df.columns:
        if col.endswith('_retrograde'):
            # Replace '1.0' values with 1 and all others incl. empty cells with 0
            df[col] = df[col].apply(lambda x: 1 if str(x) == '1.0' else 0)
